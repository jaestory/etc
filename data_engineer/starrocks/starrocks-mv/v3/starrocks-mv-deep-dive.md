...
- **FE가 "base 테이블의 어떤 파티션/snapshot이 어디까지 MV에 반영됐는가"를 추적한다.** 이 버전 맵이 증분 refresh와 rewrite consistency 판정의 공통 기반이다.
...
- **파티셔닝**: base 테이블 파티션과의 매핑이 PCT 증분 refresh의 전제다 (5.3절). 시간 컬럼에 `date_trunc()`를 써서 단위를 바꿀 수 있고(일 → 월 등), 다중 컬럼으로 파티셔닝된 Iceberg/Hive 테이블과 파티션 매핑을 컬럼 조합 단위로 직접 대응시키는 것(원문: align)도 지원된다 (v3.5+).
...
### 5.2 실행 구조: Task → TaskRun 라이프사이클

refresh는 FE의 Task 프레임워크 위에서 실행된다. MV를 생성하면 FE에 refresh Task가 등록되고, 트리거될 때마다 TaskRun이 생성되어 아래 라이프사이클을 탄다.

![Refresh 실행 구조](images/04-refresh-task-flow.png)

**① Task 등록과 트리거 (FE TaskManager)** — MV 생성 시 FE에 해당 MV의 Task가 등록된다 (`task_runs`의 task_name이 `mv-<id>` 형태인 이유). 스케줄(`EVERY`)은 FE 리더의 스케줄러가 관리하고 `START` 시각이 주기의 기준점이다. 트리거 소스는 스케줄 도래, 수동 `REFRESH`, (native base 한정) 데이터 변경이다.

**② TaskRun 생성·큐잉** — 트리거가 발생하면 TaskRun이 만들어져 PENDING 큐에 들어간다. 큐잉 규칙 세 가지: (1) **한 MV의 refresh는 동시에 1개만 실행**된다. (2) 같은 MV의 PENDING TaskRun이 이미 있으면 새 트리거는 별도 실행 없이 **MERGED 상태로 병합**된다 — 대기분 없이 RUNNING만 있으면 그 완료를 기다렸다가 실행된다. 짧은 주기 스케줄이 밀려도 refresh가 중복 폭주하지 않는 이유다. (3) refresh TaskRun의 기본 우선순위는 LOWEST로, 서빙 쿼리보다 낮은 우선순위로 실행된다. 진단 팁: `task_runs`의 `CREATE_TIME`(생성)과 `PROCESS_START_TIME`(실제 처리 시작)의 차이가 큐 대기 시간이다 — refresh가 늦을 때 "계산이 느린 것인가, 줄이 밀린 것인가"를 구분하는 1차 지표.

**③ 갱신 필요 파티션 계산** — 큐에서 나와 처리가 시작되면(RUNNING) 먼저 "무엇을 갱신할지"를 계산한다. 마지막 성공 refresh가 기록해둔 **base 파티션 버전 맵**과 현재 상태를 비교해(native: 파티션 visible version, Iceberg: 메타데이터 캐시 경유 snapshot·파티션 변경 정보) 변경된 base 파티션을 찾고, 파티션 매핑(5.3절)을 통해 갱신 대상 MV 파티션 집합을 산출한다. 이 위에 제약이 적용된다: 수동 refresh의 `PARTITION` 범위, `FORCE`(비교 생략, 지정 범위 전체), `auto_refresh_partitions_limit`(상한 — 실제 변경이 더 적으면 변경분만 대상), TTL/retention 범위. **산출 결과가 공집합이면 refresh는 스킵된다** — 스케줄이 돌아도 변경이 없으면 실제 작업이 없는 이유가 이 단계에 있다. 산출물(대상 MV 파티션 목록, 파티션 경계, force 여부, 플랜 빌더 진단)은 `EXTRA_MESSAGE`에 기록된다 (파티션 목록은 기본 100개까지 기록 후 잘림).

**④ 배치 실행 (BE/CN)** — 대상 파티션 집합을 `partition_refresh_number` 단위로 분할하고(리소스·데이터량 기반 adaptive 분할 지원), **각 배치가 하나의 task run(sub-task)** 으로 실행된다. 각 배치는 정의 쿼리에 해당 파티션 술어를 붙인 INSERT OVERWRITE 문이며, 일반 쿼리와 동일한 실행 경로를 타므로 해당 문장의 query profile로 시간·리소스를 분석할 수 있다. 파티션 교체는 원자적이어서 부분 반영 상태가 노출되지 않는다.

**⑤ 상태 확정과 진행 보존** — 배치가 성공할 때마다 그 파티션들의 버전 맵이 **즉시 갱신**된 뒤 다음 배치로 넘어간다. 덕분에 중간에 실패해도 성공한 배치의 진행은 보존되고, 다음 실행은 남은 변경 파티션만 처리한다. 전체 refresh의 성공 판정은 v3.3부터 `SHOW MATERIALIZED VIEWS`가 한 refresh를 구성하는 모든 sub-task run을 추적해 **전부 성공일 때만** `last_refresh_state = SUCCESS`를 반환한다.

**관측 (모니터링 관점)** — 라이프사이클 각 단계를 들여다보는 창구: 상태 요약은 `SHOW MATERIALIZED VIEWS`, 개별 실행 이력은 `information_schema.task_runs`(STATE: PENDING / RUNNING / SUCCESS / FAILED / MERGED), 단계 ③의 산출물·실행 상세는 `EXTRA_MESSAGE`, 단계 ④의 리소스 분석은 INSERT OVERWRITE의 query profile. 운영 대시보드의 1차 데이터 소스다.
...
> **문서상 주의 표기**: 현행 공식 문서는 Iceberg의 파티션 수준 변경 감지에 대해 "현재 Iceberg V1 테이블만 지원"이라고 명시하고 있다. 우리처럼 V2(및 V3) 테이블을 쓰는 환경에서는 파티션 수준 감지가 기대대로 동작하는지 반드시 실측해야 한다 (9장 실험 3). V2에서 감지가 안 되면 PCT의 이점(부분 refresh + 파티션 단위 consistency 판정)이 축소된다.

파티션 수준 감지가 되면 두 가지가 좋아진다: (1) 변경된 파티션만 refresh → 리소스 절약, (2) rewrite 시 "이 파티션은 stale하다"를 파티션 단위로 판정 → consistency 게이팅이 세밀해진다 (6.4절).
...
- **다중 컬럼 파티션 매핑** (v3.5+): base가 `(event_date, country)`처럼 다중 컬럼 파티션이면 MV도 `PARTITION BY (event_date, country)`로 동일 구조로 맞출 수 있다. 이러면 `(2026-07-02, JP)` 하나만 변경 시 해당 조합의 MV 파티션만 refresh되고 consistency 판정도 같은 정밀도로 내려진다. MV를 `event_date`만으로 파티셔닝하는 N:1 롤업 매핑도 가능하다.
- **조인 시 매핑은 테이블 단위로 갈린다**: 매핑 기준은 "그 테이블의 파티션 컬럼이 MV의 PARTITION BY 표현식까지 추적 가능한가"다. fact + dimension 조인에서 MV가 fact의 파티션 컬럼으로 파티셔닝되면 매핑은 fact와만 성립한다.
- **비매핑 테이블의 변경 = 전체 refresh**: dim처럼 매핑되지 않은 테이블의 변경은 특정 MV 파티션에 귀속할 수 없으므로(모든 파티션의 조인 결과에 영향) 전체 파티션이 refresh 대상이 된다. `excluded_trigger_tables`에 dim을 넣어 refresh 사유에서 제외하는 패턴이 일반적이며, 트레이드오프로 dim 변경분은 이후 갱신되는 파티션에만 반영되고 갱신 안 된 과거 파티션은 옛 dim 값을 유지한다.
- **다중 fact 테이블 매핑** (v3.3+): 복수 fact가 같은 단위의 파티션 컬럼을 갖고 **그 파티션 컬럼끼리 equi-join(또는 UNION ALL 위치 정렬)으로 연결**되어 있으면 MV 파티션이 양쪽 모두에 매핑된다 (예: `o.order_dt = r.refund_dt` 조인 → 어느 fact가 변해도 해당 날짜 MV 파티션만 갱신). 파티션 컬럼이 조인 키에 없으면 한쪽만 매핑되고 나머지는 비매핑 테이블로 취급된다.
...
**Text-based rewrite의 내부 손잡이**: FE 설정 `enable_materialized_view_text_based_rewrite`(기본 on)가 MV 생성 시 AST 구축 여부를 제어하고, 세션 변수 `materialized_view_subquery_text_match_max_count`(기본 4)가 서브쿼리 AST 비교 횟수 상한을 정한다 — 올리면 매칭 범위가 넓어지는 대신 플래닝 시간이 늘어난다. text-based rewrite도 consistency 요건(`query_rewrite_consistency`)을 통과한 MV만 쓸 수 있다.

### 6.4 Consistency·Staleness 게이팅

> **용어**: 공식 문서는 rewrite 시 base 변경 여부를 확인하는 동작을 **data consistency check**(속성 `query_rewrite_consistency`)로, 지연 허용 기반 rewrite를 **staleness rewrite**(속성 `mv_rewrite_staleness_second`)로 부른다. 이 문서의 "consistency 게이트"는 이 두 메커니즘을 합쳐 부르는 표현이다.
...
- **자주 겪는 불성립 원인**:
  ...
  - external base의 변경 감지로 consistency 게이트 탈락 (`staleness`/`loose` 미설정 상태)
...
- **메타데이터 캐시 지연 주의**: FE의 Iceberg 메타데이터 캐시가 오래되면 "변경이 있었는데 감지 못 함(consistency 게이트 오판)" 또는 "불필요한 full 판정"이 생길 수 있다. 변경 감지 이상 징후가 보이면 catalog 메타데이터 캐시 설정(TTL, refresh 주기)부터 점검한다.
...
4. **Consistency 게이팅과 staleness**: base 변경 직후 rewrite 탈락 확인 → `mv_rewrite_staleness_second` 설정 후 rewrite 복귀 확인.
